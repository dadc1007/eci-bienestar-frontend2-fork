    ...
    return filtered.map(c => {
      ...
      return (
        ...
        >
          ...
          <div className="text-xs">{c.startTime} - {c.endTime}</div>
              ...
              <td className="align-top w-20">
                <div className="relative h-[30rem]">
                  {timeSlots.map((time, idx) => (
                    <div key={idx} className="h-16 text-xs text-black px-2 border-b border-gray-300">{time}</div>
                  ))}
                </div>
              </td>
